...
class GemTest < Minitest::Test
  ...
  test "creates gems with defaults" do
    ...
    assert gem_root.join(".github/workflows/ruby-tests.yml").file?
    ...
    workflow_yml = YAML.load_file(
      gem_root.join(".github/workflows/ruby-tests.yml")
    )
  ...
  test "creates gems using custom name" do
    capture_io do
      run_command %W[#{gem_root} --name nicegem --type gem]
    end
  ...
  test "creates gems using custom namespace (NiceGem)" do
    capture_io do
      run_command %W[#{gem_root} --namespace NiceGem --type gem]
    end
  ...
  test "creates gems using custom namespace (HEYHO::LetsGo)" do
    capture_io do
      run_command %W[#{gem_root} --namespace HEYHO::LetsGo --type gem]
    end
  ...
  test "creates gems using custom namespace and name (ramones/HEYHO::LetsGo)" do
    capture_io do
      run_command %W[
        #{gem_root} --name ramones --namespace HEYHO::LetsGo --type gem
      ]
    end
  ...
  test "creates gems with binary" do
    capture_io do
      run_command %W[#{gem_root} --bin mygem --type gem]
    end
  ...
  test "creates gems with custom rubies" do
    capture_io do
      run_command %W[#{gem_root} --ruby-versions 2.6.3 2.7 --type gem]
    end
